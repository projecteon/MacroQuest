...
function xpGainedEvent(line)
   -- Experience calculation and reporting
   if(doReportXP) then
      print(string.format(">> XP-Delta: REG (%s%), AA (%s%), LDR (%s%)"), me.PctExp() - xp, me.PctAAExp() - aaXP, 0 - leaderXP)
   end
   ...
   print(string.format("%s Handins - %s min : %s sec"), handinCount, runTimeMinutes, runTimeSeconds)
end

mq.event('xpgained', 'You gain#*#experience#*#', xpGainedEvent)

mq.cmdf("/target npc %s", handinNPC)
...
print(string.format("Ran out of %s", turninItem))
